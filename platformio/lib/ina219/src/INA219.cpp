...
*/
/**************************************************************************/
INA219::INA219(I2CInterface* i2c) : _i2c(i2c) {
...
bool INA219::wireWriteRegister(uint8_t reg, uint16_t value) {
    ...
    return _i2c->writeReg(I2C_ADDRESS1 << 1, reg, buffer, 2);
}
...
bool INA219::wireReadRegister(uint8_t reg, uint16_t* value) {
    uint8_t buffer[2];
    bool success = _i2c->readReg(I2C_ADDRESS1 << 1, reg, buffer, 2);
...
int16_t INA219::getBusVoltage_raw() {
    uint16_t value = 0;
    if (!wireReadRegister(REG_BUSVOLTAGE, &value)) {
        return INT16_MAX;
    }

...
uint32_t tim2_ch1_IC_Val1 = 0;
uint32_t tim2_ch2_IC_Val1 = 0;
uint32_t tim2_ch1_IC_Val2 = 0;
uint32_t tim2_ch2_IC_Val2 = 0;
float tim2_ch1_Difference = 0;
float tim2_ch2_Difference = 0;
int tim2_ch1_Is_First_Captured = 0;
int tim2_ch2_Is_First_Captured = 0;

/* Measure Frequency */
float tim2_ch1_frequency = 0;
float tim2_ch2_frequency = 0;
/*
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
        if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)
        {
                if (tim2_ch1_Is_First_Captured==0) // if the first rising edge
is not captured
                {
                        tim2_ch1_IC_Val1 = HAL_TIM_ReadCapturedValue(htim,
TIM_CHANNEL_1); // read the first value tim2_ch1_Is_First_Captured = 1;  // set
the first captured as true
                }

                else   // If the first rising edge is captured, now we will
capture the second edge
                {
                        tim2_ch1_IC_Val2 = HAL_TIM_ReadCapturedValue(htim,
TIM_CHANNEL_1);  // read second value

                        if (tim2_ch1_IC_Val2 > tim2_ch1_IC_Val1)
                        {
                                tim2_ch1_Difference =
tim2_ch1_IC_Val2-tim2_ch1_IC_Val1;
                        }

                        else if (tim2_ch1_IC_Val1 > tim2_ch1_IC_Val2)
                        {
                                tim2_ch1_Difference = (0xffffffff -
tim2_ch1_IC_Val1) + tim2_ch1_IC_Val2;
                        }

                        float refClock = TIMCLOCK/(PRESCALAR);

                        tim2_ch1_frequency = refClock/tim2_ch1_Difference;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
      ...
      HAL_TIM_ACTIVE_CHANNEL_1) // if the interrupt source is channel1
  {
    ...
    {
      tim2_ch1_IC_Val1 = HAL_TIM_ReadCapturedValue(
    ...
    else // if the first is already captured
    {
      ...
      if (tim2_ch1_IC_Val2 > tim2_ch1_IC_Val1) {
        ...
        tim2_ch1_Difference =
            (0xffffffff - tim2_ch1_IC_Val1) + tim2_ch1_IC_Val2;
      }
    ...
    {
      tim2_ch2_IC_Val1 = HAL_TIM_ReadCapturedValue(
    ...
    } else                            // if the first is already captured
    {
      ...
      if (tim2_ch2_IC_Val2 > tim2_ch2_IC_Val1) {
        ...
        tim2_ch2_Difference =
            (0xffffffff - tim2_ch2_IC_Val1) + tim2_ch2_IC_Val2;
      }
